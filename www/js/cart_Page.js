...
function loadprof() {
    ...
    for (i = 0; i < parsedata.length; i += 2) {
        ...
        if (parsedata[i].fields.brand) {} else parsedata[i].fields.brand = 'Dudma'

        if (parsedata[i + 1].fields.brand) {} else parsedata[i + 1].fields.brand = 'Dudma'

        if (parsedata[i + 1].fields.price) {} else
            parsedata[i + 1].fields.price = '100'

        if (parsedata[i + 1].fields.description) {} else
            parsedata[i + 1].fields.description = 'beautiful'
        if (parsedata[i].fields.description) {} else
            parsedata[i].fields.description = 'beautiful'
...
<div class="item">\
  ...
  <div class="carousel-inner" role="listbox">\
                            <div class="item active">\
                                <img src="' + img20 + '" class="img-responsive items" id="' + parsedata[i + 1].fields.id + '">\
                            </div>\
<div class="item">\
